...
def main() -> None:
    parser = argparse.ArgumentParser(description="Process a CSV file with filtering and aggregation.")
    parser.add_argument("file", type=str, help="Path to the CSV file")
    parser.add_argument("--filter", nargs=3, metavar=("COLUMN", "OPERATOR", "VALUE"),
                        help="Filter rows (operators: eq, lt, gt)")
    parser.add_argument("--aggregate", nargs=2, metavar=("COLUMN", "AGG_TYPE"),
                        help="Aggregate a column (types: avg, min, max)")
    parser.add_argument("--order-by", nargs=2, metavar=("COLUMN", "ORDER"),
